...
def compute_val_score(val_logs: Dict[str, Any], args) -> float:
    ...
    score = acc + beta * f1 - alpha * (SP+EO for sex/age)
    """
...
def _select_threshold(labels_np: np.ndarray, probs_np: np.ndarray, args, fixed_thr: float):
    """
    Returns:
      thr_used, mode_used, extra_diag
    """
    if labels_np.size == 0 or probs_np.size == 0:
        return float(fixed_thr), "fixed", {
            "best_thr_f1": float("nan"),
            "best_f1": float("nan"),
            "best_thr_acc": float("nan"),
            "best_acc": float("nan"),
            "best_acc_f1": float("nan"),
        }

    mode = str(getattr(args, "eval_thr_mode", "best_f1")).lower() if args is not None else "best_f1"
    grid_mode = str(getattr(args, "eval_thr_grid", "coarse")).lower() if args is not None else "coarse"
    ...
    for t in thrs:
        pred = (probs_np >= t).astype(np.int64)
        f1v = f1_score(labels_np, pred, zero_division=0)
        if f1v > best_f1 + 1e-12:
            best_f1 = float(f1v)
            best_thr_f1 = float(t)

    # best_acc (NEW: use best_threshold_by_acc)
    best_thr_acc, best_acc, best_acc_f1 = best_threshold_by_acc(labels_np, probs_np, thrs)

    if mode == "fixed":
        thr_used, mode_used = float(fixed_thr), "fixed"
    elif mode in ["best_acc", "acc"]:
        thr_used, mode_used = float(best_thr_acc), "best_acc"
    else:
        thr_used, mode_used = float(best_thr_f1), "best_f1"
    ...
    extra = {
        ...
        "best_acc_f1": float(best_acc_f1),
    }
    return thr_used, mode_used, extra
...
def eval_epoch(
    ...
    thr_used, mode_used, extra_diag = _select_threshold(labels_np, probs_np, args=args, fixed_thr=thr_fixed)
